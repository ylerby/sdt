...
func (a *App) UpdateTransactionHandler(w http.ResponseWriter, r *http.Request) {
	var currentRequestBody requests.UpdateTransactionRequestBody
	err := json.NewDecoder(r.Body).Decode(&currentRequestBody)
	if err != nil {
		res, err := json.Marshal(schemas.Response{
			Data:          nil,
	...
	a.Database.UpdateTransactionRecord(currentRequestBody)
	res, err := json.Marshal(schemas.Response{
...
func (a *App) DeleteTransactionHandler(w http.ResponseWriter, r *http.Request) {
	var currentRequestBody requests.DeleteTransactionRequestBody
	err := json.NewDecoder(r.Body).Decode(&currentRequestBody)
	if err != nil {
		res, err := json.Marshal(schemas.Response{
			Data:          nil,
	...
	a.Database.DeleteTransactionRecord(currentRequestBody.ID)
	res, err := json.Marshal(schemas.Response{
		Data:          "Запись успешно обновлена",
		ResponseError: "",
	})
